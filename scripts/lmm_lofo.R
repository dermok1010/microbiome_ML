...
predict_kernel_holdout <- function(beta_vec, sigma_g2, sigma_e2, dat, mask, Kfull) {
  X <- model.matrix(~ SEX + Age_in_months + weight + main_breed, dat)
...
fit_lofo_once <- function(holdout_flock) {
  ...
  fit <- mmer(
    fixed  = y ~ SEX + Age_in_months + weight + main_breed,
  ...
  X_cols   <- colnames(model.matrix(~ SEX + Age_in_months + weight + main_breed, dat))
...
write.csv(preds_lofo,       "lofo_predictions.csv",       row.names = FALSE)
write.csv(metrics_by_flock, "lofo_metrics_by_flock.csv",  row.names = FALSE)
write.csv(overall,          "lofo_overall_metrics.csv",   row.names = FALSE)
